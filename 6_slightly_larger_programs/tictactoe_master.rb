...
def smart_move(hsh_available_moves)
  hsh_corner_moves_available = hsh_available_moves.keys.select do |move|
    hsh_available_moves[move].inject(:+).even?
  end
  selected_move = if hsh_available_moves.keys.include?('b2')
                    'b2'
                  elsif !hsh_corner_moves_available.empty?
                    hsh_corner_moves_available.sample
                  else
                    hsh_available_moves.keys.sample
                  end
  selected_move
end
...
def generate_move(hsh_available_moves, arr_board, difficulty, me = 'O')
  ...
  # else, for each of my options, see what move opponent would take
  if selected_move == ''
    selected_move = if difficulty == 2
                      hsh_available_moves.keys.sample
                    else
                      smart_move(hsh_available_moves)
                    end
  end
...
loop do
  ...
  loop do
    ...
    prompt "Computer chooses #{comp_move.capitalize}" unless comp_move == ''
    ...
    comp_move = generate_move(available_moves, board, difficulty)
    record_move(available_moves, board, comp_move, 'O')
